...
Stacja jest zablokowana. Musisz wysłać sygnał autoryzacyjny bezpośrednio do procesora. Zaprogramuj Arduino tak, aby co pół sekundy wysyłało kod autoryzacji `LOGIN:XX` do komputera.
...
*   **`Serial.begin(9600)`:** Ustawia prędkość "rozmowy" na 9600 bitów na sekundę. Oba urządzenia muszą mieć ustawioną tę samą prędkość.
...
*   **`delay(500)`:** Wstrzymuje program. Bez tego Arduino wysyłałoby dane tak szybko, że mogłoby zapchać bufor komputera. Długość pauzy ustawiamy w milisekundach (1s = 1000ms).
...
```
*   **`Piny GPIO (General Purpose Input/Output)`** To "nóżki" procesora, które mogą być albo przełącznikiem (Output), albo czujnikiem (Input).
...
*   **`Serial.available()`:** Sprawdza, czy w buforze (skrzynce pocztowej) Arduino czekają jakieś dane z komputera.
*   **`Serial.readStringUntil('\n')`:** Dane czekają w kolejce po jednym znaku. Żeby odczytać całą wiadomość będziemy sprawdzać znaki tak długo, aż nie natrafimy na '\n'.
...
### 📝 Instrukcja dla ucznia:
Łącze radiowe jest rozstrojone. Na ekranie widzisz chaotyczny, poruszający się kształt wektora. Podłącz potencjometr pod pin analogowy. Wysyłaj jego wartość w formacie `OFFSET:WARTOŚĆ`. Kręć potencjometrem powoli i obserwuj ekran. Musisz znaleźć "częstotliwość", przy której kształt stanie się stabilny i zielony.
...
```
*   **`ADC (Analog-to-Digital Converter).`** Potencjometr zmienia napięcie (0-5V). Arduino nie rozumie płynnego napięcia, więc zamienia je na liczbę od 0 do 1023. To właśnie robi ADC.
...
```
*   **`PWM (Pulse Width Modulation).`** Buzzer wydaje dźwięk, bo Arduino bardzo szybko włącza i wyłącza prąd, powodując drgania blaszki wewnątrz buzzera.
...
### 📝 Instrukcja dla ucznia:
Ostatni krok. Przy wypadku nastąpiło zwarcie na czujniku temperatury przez co się przepalił, a musimy monitorować temperaturę stacji. Podłącz czujnik temperatury BME280 (SDA do A4, SCL do A5). Użyj biblioteki, aby odczytać temperaturę i wyślij ją jako `TEMP:XX.X`. W terminalu ustaw suwak docelowy i sprawdź, czy system przejdzie w stan STABILIZED.

### 🔑 Rozwiązanie i wyjaśnienie (Dla prowadzącego):

**Zainstalować bibliotekę** `Adafruit BME280 Library by Adafruit` (Tools -> Manage Libraries -> wyszukać 'Adafruit_BME280' -> install) 
...
*   **Biblioteki:** Nie musimy za każdym razem pisać kodu do obsługi skomplikowanych protokołów czujnika – używamy gotowych funkcji stworzonych przez innych programistów.
*   **Histereza (W aplikacji):** Termostat nie włącza się i nie wyłącza idealnie na granicy (np. 22.0°C), bo by wariował i skakał. Aplikacja czeka, aż temperatura spadnie/wrośnie o 1 stopień (martwa strefa), zanim zmieni stan.

---

## 🛠 Hinty, FAQ i Troubleshooting:
...
2.  Jeśli nie łączy prawdopodobnie połączenie jest otwarte gdzieś indziej. 
3.  **Błąd BME280:** Jeśli czujnik nie odpowiada, najczęstszym błędem jest zamiana kabli SDA/SCL lub zły adres (zmień 0x76 na 0x77).
4.  **Monitoruj logi:** Na dole aplikacji webowej jest czarny pasek z logami. Tam zobaczysz każdą ramkę odebraną z Arduino – to najszybszy sposób, by pokazać uczniowi, czy jego kod działa poprawnie.
